...
function send_photo(uid, file_id, caption){
  var payload = {
    method: "sendPhoto",
    chat_id: String(uid),
    photo: file_id,
...
function delete_msg(uid, message){
  var response = UrlFetchApp.fetch("https://api.telegram.org/bot" + tg_token + "/deleteMessage?message_id=" + message.message_id + "&chat_id=" + uid + "&parse_mode=HTML");
...
function donation(uid){
  ...
  send_msg(uid, "目前提供 Taiwan Pay 和 Line Pay 抖內管道，不限金額。謝謝乾爹乾媽。")
...
function check_and_download_json(){
  ...
  if(!isFileExists(today_json_name, my_schedule_folder_id)){
    schedule_json_list = []
    ...
    downloadFile2GD(today_json_name, file_url, my_schedule_folder_id)
  }
  else if(!isFileExists(tomorrow_json_name, my_schedule_folder_id)){
    ...
    downloadFile2GD(tomorrow_json_name, file_url, my_schedule_folder_id)
  }
  delteFile(yesterday_json_name, my_schedule_folder_id)
  return 0
}
...
function setWebhook(){
  ...
  var appUrl = "https://this_gas_exec_url"
